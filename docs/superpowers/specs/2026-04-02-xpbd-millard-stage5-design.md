...
**思路**：`target = lm - σ₀·f_total/k_base`，`stiffness = k_base`，使约束力精确等于 Millard 力。
...
则约束产生的能量密度 ≈ Ψ_Millard(lm)，与 Millard 本构模型精确一致。

**关键优势**：
1. **Ψ 已有闭式解**：Millard quintic Bezier 的能量积分是 10 次多项式，已在 Stage 1 实现
2. **GPU-only**：C 从当前 lm 直接计算，无需 CPU 端平衡反求
3. **物理一致性**：约束力 = ∂E/∂x ∝ dΨ/dlm = f_hill(lm)，自动恢复 Hill 力-长度关系
...
主动能量密度：
```
Ψ_active(lm, a) = σ₀ · a · ∫_{lm_opt}^{lm} f_L(lm') dlm'
```

**问题**：f_L 在 lm=lm_opt=1.0 处取最大值 1.0，在两侧下降。积分 ∫₁^{lm} f_L(lm') dlm'：
- lm > 1: 正值（沿下降段积分）
- lm < 1: 负值（f_L 在 ascending 段单调递增，从1反向积分）

**偏移处理**：定义 Ψ_active 的参考点为 f_L 的左肩 lm_min（Millard: 0.4441）：
```
Ψ_active(lm, a) = σ₀ · a · ∫_{lm_min}^{lm} f_L(lm') dlm'
```

由于 f_L(lm_min) = y_low = 0.1 > 0（Millard 肩部非零），且 f_L 在 [lm_min, lm_max] 上恒正，所以 Ψ_active ≥ 0 对所有 lm ≥ lm_min。

**约束的物理含义**：C_active = √(2·Ψ_active) 编码了"纤维偏离最短长度的能量代价"。求解器会驱动纤维向 lm_min 方向收缩（能量最小化），但被 volume/arap/attach 约束平衡在物理合理的长度。
...
```
GPU: lm = ||F·d||                          (已有)
GPU: Ψ_L  = millard_energy_eval_wp(lm)    (新增：10次多项式求值)
...
def millard_energy_eval_wp(
    ...
    """Evaluate Millard energy Ψ_L(lm) via closed-form 10th-degree polynomial.
...
if psi_act > 1.0e-12:
    ...
    # dC/dlm 需要 f_L(lm)，已有 millard_eval_wp
    f_L_val = millard_eval_wp(lm_tilde, fl_data...)
    grad_C_wrt_lm = sigma0 * acti * f_L_val / C_act
...
- f_L 的能量参考点设为 lm_min = 0.4441（确保 Ψ_L ≥ 0）
...
Ψ_L(lm) = ∫_{lm_min}^{lm} f_L(lm')dlm'。f_L 有界（最大值 1.0），所以 Ψ_L(lm_max) = ∫_{0.44}^{1.81} f_L dlm' ≈ 0.7（有限值）。C_act = √(2σ₀·a·Ψ_L) 有界。

### 奇异性处理

当 Ψ → 0 时，C → 0 但 dC/dlm → ∞（梯度爆炸）。处理方式：
...
在关键路径上打印 log 以方便调试，包括但不限于：
- CPU 验证脚本：每迭代打印 lm、C、dC/dlm、Δlm
- GPU kernel：在 CPU 模式运行时打印首个 tet 的 lm、Ψ_L、C、∇C、Δλ_xpbd
- 示例运行：每步打印平均 lm、球位置/肘角等关键指标

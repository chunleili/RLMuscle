...
- 力函数 f(lm) 可精确求值（无需 exp/log）
- 能量积分 Ψ(lm) = ∫f ds 有 **闭式解**（10次多项式）
...
| 曲线 | 含义 | 段数 | 定义域 |
|------|------|------|--------|
| f_L(lm) | 主动力-长度曲线 | 5 段 | [0.4441, 1.8123] |
| f_PE(lm) | 被动力-长度曲线 | 2 段 | [1.0, 1.7] |

其中 lm = 当前纤维长度 / 最优纤维长度（归一化纤维拉伸）。
...
| 参数 | 值 | 含义 |
|------|-----|------|
| e_zero | 0.0 | 开始产生力的 strain（lm=1.0 时） |
| e_iso | 0.7 | 归一化力=1.0 时的 strain（lm=1.7） |
| k_low | 0.2 | 低力区域斜率 |
...
域外行为：lm ≤ 1.0 时 f_PE = 0；lm > 1.7 时线性外推（斜率 k_iso）。
...
solve_tetfibermillard_kernel (per tet):
  1. 变形梯度 F = Ds · Dm⁻¹
  2. 纤维拉伸 lm = ||F·d|| (d = 材料空间纤维方向)
  3. Millard 曲线求值:
     f_L = millard_eval_wp(lm, fl_data...)
     Ψ_L = millard_energy_eval_wp(lm, fl_energy_data...)
  4. 能量约束: C = √(2·σ₀·a·Ψ_L)
  5. 约束梯度 (链式法则):
     dC/dlm = σ₀·a·f_L / C  (when C > ε)
     ∂lm/∂x_i = ∂||F·d||/∂x_i  (几何梯度，从 tet 形状推导)
     ∇C_i = (dC/dlm) · (∂lm/∂x_i)  (对顶点位置的 3D 梯度)
  6. XPBD 约束更新:
     w_sum = Σ_i m_i⁻¹ ||∇C_ij||²
     Δλ_xpbd_j = -(C+ α/dt²*lambda) / (w_sum + α/dt²)
```

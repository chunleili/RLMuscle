...
> - `D:\Dev\OpenSimExample\OpenSimExample\vbd_muscle\vbd_muscle.py` — 现有基于 PyTorch 的 VBD 肌肉实现（不直接复用，但设计思路借鉴）
...
1. **在 Newton 子模块创建 `feat/vbd-muscle` 分支**，主项目追踪该分支
...
```
examples/muscle_contraction.py                ← Example 层（算例入口）
...
external/newton/newton/_src/                  ← Model + Solver 层（Newton 内部修改）
  ...
  │     ├── add_vmuscle_tetrahedron(): 新方法，添加带纤维属性的四面体
...
| 文件 | 修改类型 | 修改内容 |
|------|---------|---------|
| `_src/sim/builder.py` | 插入 | `__init__` 新增 `vmuscle_*` 列表；新增 `add_vmuscle_tetrahedron()`；`finalize()` 写入数组 |
...
self.vmuscle_tet_fiber_dirs = []      # (n_tets,) vec3, rest config unit fiber direction
self.vmuscle_tet_sigma0 = []          # (n_tets,) float, peak isometric stress [Pa]
self.vmuscle_tet_activations = []     # (n_tets,) float, activation level [0,1]
...
```python
def add_vmuscle_tetrahedron(self, i, j, k, l,
                             k_mu, k_lambda, k_damp=0.0,
                             fiber_dir=(0,0,0), sigma0=0.0):
    """Add a tetrahedron with volumetric muscle fiber properties.

    The base isotropic material (Neo-Hookean) is handled by the standard
    tet elasticity. The fiber direction and sigma0 define the anisotropic
    Hill-type muscle energy that is added on top.

    Args:
        i, j, k, l: Vertex indices.
        k_mu: Shear modulus for base Neo-Hookean material.
        k_lambda: Bulk modulus for base Neo-Hookean material.
        k_damp: Damping coefficient.
        fiber_dir: Unit fiber direction in rest configuration.
        sigma0: Peak isometric stress [Pa].
    """
    vol = self.add_tetrahedron(i, j, k, l, k_mu=k_mu, k_lambda=k_lambda, k_damp=k_damp)
    self.vmuscle_tet_fiber_dirs.append(tuple(fiber_dir))
    self.vmuscle_tet_sigma0.append(float(sigma0))
    self.vmuscle_tet_activations.append(0.0)
    return vol
```
...
```python
# volumetric muscle arrays
if self.vmuscle_tet_fiber_dirs:
    m.vmuscle_tet_fiber_dirs = wp.array(
        self.vmuscle_tet_fiber_dirs, dtype=wp.vec3, device=device, requires_grad=requires_grad)
    m.vmuscle_tet_sigma0 = wp.array(
        self.vmuscle_tet_sigma0, dtype=wp.float32, device=device)
    m.vmuscle_tet_activations = wp.array(
        self.vmuscle_tet_activations, dtype=wp.float32, device=device)
    m.vmuscle_max_contraction_velocity = self.vmuscle_max_contraction_velocity
    m.vmuscle_count = len(self.vmuscle_tet_fiber_dirs)
else:
    m.vmuscle_count = 0
```

> **注意**：`vmuscle_tet_fiber_dirs` 等数组的长度等于所有调用 `add_vmuscle_tetrahedron()` 的次数，**不是** `model.tet_count`。需要维护一个 `vmuscle_tet_ids` 列表记录每个 vmuscle 四面体在全局 tet 数组中的 index，供 kernel 查找。或者直接让 vmuscle 数组长度 = `tet_count`，非肌肉四面体的 sigma0 = 0（更简单，kernel 中用 sigma0 > 0 判断跳过）。
>
> **推荐方案**：vmuscle 数组长度 = `tet_count`，非肌肉四面体填 zero fiber + sigma0=0。这样 kernel 索引与 tet_id 直接对应，无需额外映射。`add_tetrahedron()` 保持不变，`add_vmuscle_tetrahedron()` 在调用 `add_tetrahedron()` 后记录同步。需要在 `add_tetrahedron()` 中同步追加 zero 占位，或在 `finalize()` 中 padding 到 tet_count 长度。
...
### 4.6 src/VMuscle/mesh_utils.py — add_soft_vmuscle_mesh()

批量导入 USD 肌肉网格的 helper 函数，放在 Example 层（`src/VMuscle/`），不侵入 Newton。

```python
def add_soft_vmuscle_mesh(
    builder,                      # newton.ModelBuilder
    mesh: newton.TetMesh,         # 含 custom_attributes 的 TetMesh
    cfg,                          # SimConfig from load_config()
    pos=wp.vec3(0, 0, 0),
    rot=wp.quat_identity(),
    scale=1.0,
    vel=wp.vec3(0, 0, 0),
):
    """Import a TetMesh with volumetric muscle properties.

    Reads 'materialW', 'muscle_id', 'tendonmask' from mesh.custom_attributes.
    Converts per-vertex attributes to per-tet and populates builder.vmuscle_* arrays.
    ...
    Args:
        ...
        pos, rot, scale, vel: Transform for add_soft_mesh().
    """
```

**内部流程**：

1. 记录 `tet_offset = len(builder.tet_indices)` 和 `particle_offset`
2. 调用 `builder.add_soft_mesh(mesh, pos, rot, scale, vel)`
3. 从 `mesh.custom_attributes` 提取 `materialW`→fiber_dir, `muscle_id`, `tendonmask`
4. 遍历新增 tet，per-vertex→per-tet 转换：
   - `fiber_dir`：4 顶点平均 + 归一化
   - `muscle_id`：取第一个顶点
   - `tendonmask`：4 顶点平均，与 `cfg.tendon_threshold` 比较
5. 按 `muscle_id` 查 `cfg.muscles[str(mid)]["sigma0"]` 填充
6. tendon tet（tendonmask >= 阈值）的 `sigma0` 设为 0（无主动收缩）
7. 填充 `builder.vmuscle_tet_fiber_dirs/sigma0/activations`

此外还有其他辅助函数：

- `create_cylinder_tet_mesh(length, radius, n_length, n_radial)` — 生成圆柱体四面体网格（用于测试/快速原型）
- `assign_fiber_directions(vertices, tets, axis)` — 沿指定轴为每个四面体赋纤维方向

### 4.7 examples/muscle_contraction.py（Example 层算例入口）
...
- `add_vmuscle_tetrahedron()` 仍保留用于底层逐 tet 添加（`add_soft_vmuscle_mesh` 内部不调用它，直接批量填充 vmuscle_* 列表以避免重复调用 `add_tetrahedron`）

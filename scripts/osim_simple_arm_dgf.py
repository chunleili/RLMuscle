"""OpenSim SimpleArm with DeGrooteFregly2016Muscle.

Config-driven simulation: rigid tendon, activation dynamics, DGF muscle.
Returns dict for comparison with Millard and MuJoCo.

Usage:
    uv run python scripts/osim_simple_arm_dgf.py
    uv run python scripts/osim_simple_arm_dgf.py --config data/simpleArm/config.json
"""
...
def osim_simple_arm_dgf(cfg):
    """Build and run OpenSim SimpleArm with DGF muscle.
    ...
    print(f"[DGF] Simulating {t_end:.1f}s, F_max={mus['max_isometric_force']:.0f}N, "
    ...
    print(f"[DGF] Done: {len(times)} points, final angle={np.degrees(elbow_angles[-1]):.1f}°")

    # Save .osim and .sto
    os.makedirs("output", exist_ok=True)
    model.printToXML("output/SimpleArm_DGF.osim")
    sto_path = "output/SimpleArm_DGF_states.sto"
    osim.STOFileAdapter.write(states_table, sto_path)
    # Rename first line from default "states" to case name
    with open(sto_path, "r") as f:
        lines = f.readlines()
    lines[0] = "SimpleArm_OpenSim_DGF\n"
...
if __name__ == "__main__":
    ...
    result = osim_simple_arm_dgf(cfg)
    if result:

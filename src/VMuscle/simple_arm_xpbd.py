...
Extracted from examples/example_xpbd_coupled_simple_arm.py during refactor.
All numerical behavior is preserved exactly.
"""
...
def build_xpbd_muscle_sim(
    ...
    *,
    ...
    upload_all_constraints(sim, all_cons)
...
def run_xpbd_coupled_loop(cfg, verbose=True):
    ...
    R = rotation_matrix_from_z_to(tdu)
    ...
    sim = build_xpbd_muscle_sim(
        ...
        density=density, veldamping=veldamping,
    )
    ...
    for step in range(n_steps):
        ...
        for _sub in range(num_substeps):
            ...
            R_path = rotation_between(tdu_init, cur_tdu)

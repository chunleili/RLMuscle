...
Uses TETFIBERNORM constraint for fiber contraction on bicep geometry.

Usage:
    RUN=example_couple2 uv run main.py --auto --steps 300
    RUN=example_couple2 uv run main.py --auto --render
    RUN=example_couple2 uv run main.py --eval
"""
...
DEFAULT_CONFIG = "data/muscle/config/bicep_coupled.json"
...
def setup_couple2(
    ...
    """Initialize couple2 simulation components.
...
def _create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Warp-only muscle-bone coupling demo (TETFIBERNORM)")
...
def main():
    ...
    solver, sim, state, cfg, dt = setup_couple2(

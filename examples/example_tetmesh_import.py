...
# Register integer muscle_id (USD stores it as string, we convert to int index)
builder.add_custom_attribute(
    newton.ModelBuilder.CustomAttribute(
        name="muscle_id",
        frequency=newton.Model.AttributeFrequency.PARTICLE,
...
muscle_hash_to_name = {}  # crc32 hash -> muscle name string
muscle_name_to_hash = {}  # muscle name string -> crc32 hash
...
for prim_path in TEST_MUSCLES:
    ...
    # get_tetmesh() returns the global name list (199 entries), so we resolve

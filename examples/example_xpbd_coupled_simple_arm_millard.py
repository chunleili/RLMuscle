"""XPBD coupled SimpleArm with Millard 2012 muscle curves.

Same as example_xpbd_coupled_simple_arm.py but uses Millard quintic Bezier
force-length curves instead of DGF for the 1D Hill-type force model.
...
def xpbd_coupled_simple_arm(cfg, verbose=True):
    ...
    # --- Initialize Millard curves for 1D force model ---
    _mc = MillardCurves()
    ...
    for step in range(n_steps):
        ...
        for _sub in range(num_substeps):
            ...
            for _ in range(mj_per_xpbd):
                ...
                fl = float(_mc.fl.eval(fib_len / L_opt))
                fpe = float(_mc.fpe.eval(fib_len / L_opt))
    ...
    return {"times": np.array(times), "elbow_angles": np.array(elbow_angles),
            ...
            "max_iso_force": F_max, "muscle_type": "XPBD_Coupled_Millard"}
